...
export class RecentlyPlayedComponent implements OnInit {
  ...
  @Input() firstTeamLogo?: string;
  @Input() secondTeamLogo?: string;
  ...
  navigateToTeamPage(teamName: string | undefined) {
    this.router
  ...
  getMoreResults(): void {
    if(this.listFirstSelected?.length !== this.listFirst?.latest?.length) {
      this.moreFirstTeam()
    }

    if(this.listSecondSelected?.length !== this.listFirst?.latest?.length) {
      this.moreSecondTeam()
    }
  }
}
