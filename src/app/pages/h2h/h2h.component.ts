      ...
      this.leagueService.search(leagueSeasons[0].leagueName)
        .pipe(
          switchMap(res => {
            ...
            const metaObject = {
              title: `${this.firstTeam?.name} vs ${this.secondTeam?.name} United H2H & Predictions, Watch Live Stats |  SteveGfootball`,
              description: `H2H Predictions & Head to Head live stats for ${this.firstTeam?.name} vs ${this.secondTeam?.name} United in the Premier League. Goals Scored,BTTS, Goals Conceded, Clean Sheets, past stats analysis, results and more.`,
              keywords: `Head To Head, h2h,FTS, BTTS, Clean Sheets, Goals Scored, Corners, Top Scorers, Over 2.5, Football, Table, Stats, Results, SteveGfootball`,
            };
