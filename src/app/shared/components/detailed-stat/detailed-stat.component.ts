...
export class DetailedStatComponent extends FilterLeagueDirective {
        ...
        (res: DetailStats[]) => {
          this.detailedStats = res?.map((item) => {
            return {
              ...
              homeAwayOver:
                item.stats.over_2_5.home && item.stats.over_2_5.away
                  ? `${item.stats.over_2_5.home}-${item.stats.over_2_5.away}`
                  : "-",
              ourPredictionOver: 0,
              // homeAwayUnder: `${item.stats.under_2_5.home}-${item.stats.under_2_5.away}`,
              homeAwayUnder:
                item.stats.under_2_5.home && item.stats.under_2_5.away
                  ? `${item.stats.under_2_5.home}-${item.stats.under_2_5.away}`
                  : "-",
  ...
  sortByCard(item: string) {
    if (item === this.cardTypes.RED) {
      ...
      this.detailedStats = this.detailedStats.sort(
        (a, b) => b.yellow - a.yellow
      );
    }
  }
