...
export class TeamPlayerStatsComponent
  ...
  implements OnInit
{
  ...
  ngOnInit(): void {
    if (this.router.url.includes("league")) {
      this.leagueService
        .getLeagueSeasons(this.leagueService.league.id)
        .subscribe((res) => {
          this.seasons = res;
        });
    } else {
          ...
          map(
            (league) =>
              league.find(
                (l) =>
                  l.leagueId ===
                  (this.teamService.team?.leagueId ||
                    this.h2hService?.league?.id)
              )?.seasons
          ),
  ...
  private getPlayersStat(seasonId: number | undefined) {
    ...
    if (this.router.url.includes("league")) {
      ...
      this.teamService
        .getPlayersStat(this.teamId, seasonId)
        .pipe(takeUntil(this.subscription))
        .subscribe((res) => {
  ...
  more() {
    ...
    this.page += 1;
    this.playerStats.players = this.allPlayers.filter(
      (_, i) => this.page * this.count > i
    );
  }
