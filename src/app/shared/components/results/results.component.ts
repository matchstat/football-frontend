...
export class ResultsComponent extends FilterDirective {
  ...
  results: Fixture[] = [];
  currentTeamId: number = this.teamService.team.id;
        ...
        (res: Fixture[]) => {
          ...
          this.results = this.results.concat(
            res.map((r) => {
              let className = '';
              if (
                r.homeTeam.score !== undefined &&
                r.awayTeam.score !== undefined
              ) {
                if (
                  (r.awayTeam.name.toLowerCase() ===
                    this.title?.toLowerCase() &&
                    r.awayTeam.score > r.homeTeam.score) ||
                  (r.homeTeam.name.toLowerCase() ===
                    this.title?.toLowerCase() &&
                    r.homeTeam.score > r.awayTeam.score)
                ) {
  ...
  protected override getSeasons() {
    ...
    if (this.seasons?.length) {
      const currentYear = this.seasons.find(
        (s) => s.seasonId === new Date().getFullYear()
      );
      this.curSeason = currentYear
        ? currentYear.seasonId
        : this.seasons[0].seasonId;
    }
  }
}
