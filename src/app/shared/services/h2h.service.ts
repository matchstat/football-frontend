      ...
      data?.forEach((el: H2hInterestingInterface) => {
        if (el?.localteam_id === el.winner_team_id) {
          interestingData.push({
            winnerTeamName: el.local_team.name, winnerTeamLogo: el.local_team.logo_path, winnerTeamScore: el.localteam_score, winnerTeamId: el.localteam_id,
            failedTeamName: el.visitor_team.name, failedTeamLogo: el.visitor_team.logo_path, failedTeamScore: el.visitorteam_score, failedTeamId: el.visitorteam_id})
        } else {
          interestingData.push({
            ...
            winnerTeamId: el.visitorteam_id,
            failedTeamId: el.localteam_id
          })
        }
      })
